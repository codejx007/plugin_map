...
+ (MATagPolyline *)tagPolylineForCoordinateString:(NSString *)coordinateString
{
    ...
    MATagPolyline *polyline = [MATagPolyline polylineWithCoordinates:coordinates count:count];
...
+ (NSArray *)polylinesForSegment:(AMapSegment *)segment
{
    ...
    MAPolyline *busLinePolyline = [self polylineForBusLine:segment.buslines];
...
+ (MATagPolyline *)tagPolylineForLine:(MAPolyline *)stepPolyline lastPolyline:(MAPolyline *)lastPolyline{
    ...
    if ((endCoor.latitude != startCoor.latitude || endCoor.longitude != startCoor.longitude)){
        ...
        MATagPolyline *polyline = [MATagPolyline polylineWithCoordinates:points count:2];
        return polyline;
    }
    return nil;
}
                           ...
                           Polylines:(NSMutableArray *)polylines
{
    if (lastSegment)
    {
        ...
        MAPolyline *busLinePolyline = [self polylineForBusLine:(lastSegment).buslines];
        if (busLinePolyline != nil)
        {
